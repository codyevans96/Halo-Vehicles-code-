...
function ENT:SpawnFunction(pl, tr)
	local e = ents.Create("halov_banshee2");
...
function ENT:Initialize()
	...
	self.WeaponsTable = {};
	self.BoostSpeed = 2700;
	self.ForwardSpeed = 2700;
	self.UpSpeed = 1000;
	self.AccelSpeed = 10;
	self.CanBack = true;
	self.CanRoll = true;
	self.Cooldown = 2;
	self.CanShoot = true;
	self.Bullet = HALOCreateBulletStructure(90,"plasma");
...
if CLIENT then
	...
	function CalcView()
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("HALOV_Banshee2", NULL)
	...
	hook.Add("CalcView", "HALOV_Banshee2View", CalcView)
	...
	function HALOV_Banshee2Reticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingHALOV_Banshee2");
		local self = p:GetNWEntity("HALOV_Banshee2");
	...
	hook.Add("HUDPaint", "HALOV_Banshee2Reticle", HALOV_Banshee2Reticle)
