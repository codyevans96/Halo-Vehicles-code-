...
ENT.FireSound = Sound("weapons/cov_shoot.wav");
ENT.NextUse = {Wings = CurTime(),Use = CurTime(),Fire = CurTime(),};

AddCSLuaFile();
function ENT:SpawnFunction(pl, tr)
	local e = ents.Create("halov_seraph");
        ...
        if(IsValid(self.Pilot)) then
            if(IsValid(self.Pilot)) then 
                if(self.Pilot:KeyDown(IN_ATTACK2) and self.NextUse.FireBlast < CurTime()) then
                    ...
                    self:FireHALOV_SeraphBlast(self.BlastPositions[self.NextBlast], false, 300, 300, true, 8, Sound("weapons/banshee_bomb.wav"));
...
function ENT:FireHALOV_SeraphBlast(pos,gravity,vel,dmg,white,size,snd)
	if(self.NextUse.FireBlast < CurTime()) then
...
if CLIENT then
	...
	function CalcView()
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("HALOV_Seraph", NULL)
	...
	hook.Add("CalcView", "HALOV_SeraphView", CalcView)
	...
	function ENT:Effects()
		...
		for k,v in pairs(self.HALOV_SeraphEnginePos) do
	...
	function HALOV_SeraphReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingHALOV_Seraph");
		local self = p:GetNWEntity("HALOV_Seraph");
	...
	hook.Add("HUDPaint", "HALOV_SeraphReticle", HALOV_SeraphReticle)
