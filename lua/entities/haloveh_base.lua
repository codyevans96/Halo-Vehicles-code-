...
function ENT:Exit(kill) --####### Get out @RononDex
	...
	local p = self.Pilot;
	p:SetNWInt("HALO_Health",nil);
	p:SetNWInt("HALO_Overheat",nil);
	p:SetNWBool("HALO_Overheated",nil);	
	p:SetNWBool("HALO_Critical",nil);	
	p:SetNWBool("HALO_ShouldLock",nil);
	p:SetNWBool("HALO_Speed",nil);
	p:SetNWInt("HALO_MaxSpeed",nil);
	self.Pilot:SetNWBool("HALO_Doors",nil);
...
function ENT:Bang()
	...
	self:EmitSound(Sound("Haloexplosion.wav"),100,math.random(80,100));
	local fx = EffectData();
