...
hook.Add("PlayerEnteredVehicle","AV14SeatEnter", function(p,v)
	if(IsValid(v) and IsValid(p)) then
		if(v.IsAV14Seat) then
			p:SetNetworkedEntity("AV14",v:GetParent());
            p:SetNetworkedEntity("AV14Seat",v);
			p:SetNetworkedBool("AV14Passenger",true);
		end
	end
end);

hook.Add("PlayerLeaveVehicle", "AV14SeatExit", function(p,v)
	if(IsValid(p) and IsValid(v)) then
		if(v.IsAV14Seat) then
            if(v.AV14FrontSeat) then
			...
			p:SetNetworkedEntity("AV14",NULL);
            p:SetNetworkedEntity("AV14Seat",NULL);
			p:SetNetworkedBool("AV14Passenger",false);
		end
	end
end);
        ...
        if(IsValid(self.Pilot)) then
            if(IsValid(self.Pilot)) then 
                if(self.Pilot:KeyDown(IN_ATTACK2) and self.NextUse.FireBlast < CurTime()) then
                    ...
                    self:FireAV14Blast(self.BlastPositions[self.NextBlast], false, 250, 250, true, 8, Sound("weapons/hornet_missle.wav"));
...
function ENT:FireAV14Blast(pos,gravity,vel,dmg,white,size,snd)
...
if CLIENT then
	...
	hook.Add("ScoreboardShow","AV14ScoreDisable", function()
		local p = LocalPlayer();	
		local Flying = p:GetNWBool("AV14");
		if(Flying) then
			return false;
		end
	end)
	...
	local function CalcView()
		...
		local p = LocalPlayer();	
		local Flying = p:GetNWBool("FlyingAV14");
		local Sitting = p:GetNWBool("AV14Passenger");
		local pos, face;
		local self = p:GetNWEntity("AV14");
		...
		elseif(Sitting) then
			local v = p:GetNWEntity("AV14Seat");	
			if(IsValid(v)) then
	...
	hook.Add("CalcView", "AV14View", CalcView)
	
	hook.Add( "ShouldDrawLocalPlayer", "AV14DrawPlayerModel", function( p )
		local self = p:GetNWEntity("AV14", NULL);
		local PassengerSeat = p:GetNWEntity("AV14Seat",NULL);
		if(IsValid(self)) then
	...
	function ENT:Effects()
		...
		for k,v in pairs(self.AV14EnginePos) do
	...
	hook.Add("HUDPaint", "AV14Reticle", AV14Reticle)
